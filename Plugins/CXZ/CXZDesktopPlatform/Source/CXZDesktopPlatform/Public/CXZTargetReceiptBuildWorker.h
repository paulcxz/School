...
struct FCXZTargetReceipt;
class FCXZTargetReceiptBuildWorkerFactory;

/**
 * Globally registers a UE::DerivedData::IBuildWorkerFactory instance that runs an executable built by UnrealBuildTool.
 * UnrealBuildTool provides the executable information through a CXZTargetReceipt file.
*/
class CXZDESKTOPPLATFORM_API FCXZTargetReceiptBuildWorker
{
public:
	FCXZTargetReceiptBuildWorker(const TCHAR* TargetReceiptFilePath);
	virtual ~FCXZTargetReceiptBuildWorker();
...
private:
	...
	*/
	...
	friend class FCXZTargetReceiptBuildWorkerFactory;

	FCXZTargetReceiptBuildWorkerFactory* GetWorkerFactory();
	 ...
	 * The FCXZTargetReceiptBuildWorkerFactory type will be created and stored in the space of one of these.
