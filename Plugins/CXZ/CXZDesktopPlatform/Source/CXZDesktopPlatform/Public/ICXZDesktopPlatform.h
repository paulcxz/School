...
enum class ECXZFontImportFlags
{
	None = 0x0,						// No flags
...
ENUM_CLASS_FLAGS(ECXZFontImportFlags)
 ...
 */
class FCXZScopedSystemModalMode
{
public:
	FCXZScopedSystemModalMode()
	{
#if WITH_EDITOR
 ...
 */
class ICXZDesktopPlatform
{
public:
	/** Virtual destructor */
	virtual ~ICXZDesktopPlatform() {}

	//cxz add
	virtual bool MakeDirectory(const FString& NewDirectory) = 0;
	virtual bool RenameFile(const FString& Src,const FString& Dest) = 0;
	 ...
	 * @param Flags						Details about the dialog. See ECXZFileDialogFlags.
	 ...
	 */
	virtual bool OpenFontDialog(const void* ParentWindowHandle, FString& OutFontName, float& OutHeight, ECXZFontImportFlags& OutFlags) = 0;
	...
	*/
	 ...
	 */
	virtual const TArray<FCXZTargetInfo>& GetTargetsForProject(const FString& ProjectFile) const = 0;
	 ...
	 * @return Array of TargetInfo objects
	 */
	virtual const TArray<FCXZTargetInfo>& GetTargetsForCurrentProject() const = 0;
