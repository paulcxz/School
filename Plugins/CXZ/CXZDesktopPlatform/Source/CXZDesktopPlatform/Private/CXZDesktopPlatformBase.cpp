...
#define LOCTEXT_NAMESPACE "CXZDesktopPlatform"


FCXZDesktopPlatformBase::FCXZDesktopPlatformBase()
{
	LauncherInstallationTimestamp = FDateTime::MinValue();
}

FString FCXZDesktopPlatformBase::GetEngineDescription(const FString& Identifier)
{
...
FString FCXZDesktopPlatformBase::GetCurrentEngineIdentifier()
{
...
void FCXZDesktopPlatformBase::EnumerateLauncherEngineInstallations(TMap<FString, FString> &OutInstallations)
{
...
void FCXZDesktopPlatformBase::EnumerateLauncherSampleInstallations(TArray<FString> &OutInstallations)
{
	// Cache the launcher install list if necessary
	ReadLauncherInstallationList();
...
void FCXZDesktopPlatformBase::EnumerateLauncherSampleProjects(TArray<FString> &OutFileNames)
{
...
bool FCXZDesktopPlatformBase::GetEngineRootDirFromIdentifier(const FString &Identifier, FString &OutRootDir)
{
...
bool FCXZDesktopPlatformBase::GetEngineIdentifierFromRootDir(const FString &RootDir, FString &OutIdentifier)
{
	// Get all the installations
	TMap<FString, FString> Installations;
	EnumerateEngineInstallations(Installations);
...
bool FCXZDesktopPlatformBase::GetDefaultEngineIdentifier(FString &OutId)
{
	TMap<FString, FString> Installations;
	EnumerateEngineInstallations(Installations);
...
bool FCXZDesktopPlatformBase::GetDefaultEngineRootDir(FString &OutDirName)
{
	FString Identifier;
	return GetDefaultEngineIdentifier(Identifier) && GetEngineRootDirFromIdentifier(Identifier, OutDirName);
}

bool FCXZDesktopPlatformBase::IsPreferredEngineIdentifier(const FString &Identifier, const FString &OtherIdentifier)
{
...
bool FCXZDesktopPlatformBase::TryGetEngineVersion(const FString& RootDir, FEngineVersion& OutVersion)
{
...
bool FCXZDesktopPlatformBase::IsStockEngineRelease(const FString &Identifier)
{
	FGuid Guid;
	return !FGuid::Parse(Identifier, Guid);
}

bool FCXZDesktopPlatformBase::TryParseStockEngineVersion(const FString& Identifier, FEngineVersion& OutVersion)
{
	TCHAR* End;
...
bool FCXZDesktopPlatformBase::IsSourceDistribution(const FString &EngineRootDir)
{
...
bool FCXZDesktopPlatformBase::IsPerforceBuild(const FString &EngineRootDir)
{
	// Check for the existence of a SourceBuild.txt file
...
bool FCXZDesktopPlatformBase::IsValidRootDirectory(const FString &RootDir)
{
...
bool FCXZDesktopPlatformBase::SetEngineIdentifierForProject(const FString &ProjectFileName, const FString &InIdentifier)
{
...
bool FCXZDesktopPlatformBase::GetEngineIdentifierForProject(const FString& ProjectFileName, FString& OutIdentifier)
{
	OutIdentifier.Empty();
...
bool FCXZDesktopPlatformBase::OpenProject(const FString& ProjectFileName)
{
	FPlatformProcess::LaunchFileInDefaultExternalApplication(*ProjectFileName);
	return true;
}

bool FCXZDesktopPlatformBase::CleanGameProject(const FString& ProjectDir, FString& OutFailPath, FFeedbackContext* Warn)
{
	// Begin a task
...
bool FCXZDesktopPlatformBase::CompileGameProject(const FString& RootDir, const FString& ProjectFileName, FFeedbackContext* Warn, ECompilationResult::Type* OutResult)
{
	...
	// Run UBT
	int32 ExitCode;
	bool bResult = static_cast<ICXZDesktopPlatform*>(this)->RunUnrealBuildTool(LOCTEXT("CompilingProject", "Compiling project..."), RootDir, Arguments, Warn, ExitCode);
...
bool FCXZDesktopPlatformBase::GenerateProjectFiles(const FString& RootDir, const FString& ProjectFileName, FFeedbackContext* Warn, FString LogFilePath)
{
...
bool FCXZDesktopPlatformBase::IsUnrealBuildToolAvailable()
{
...
bool FCXZDesktopPlatformBase::InvokeUnrealBuildToolSync(const FString& InCmdLineParams, FOutputDevice &Ar, bool bSkipBuildUBT, int32& OutReturnCode, FString& OutProcOutput)
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FCXZDesktopPlatformBase::InvokeUnrealBuildToolSync);
...
FProcHandle FCXZDesktopPlatformBase::InvokeUnrealBuildToolAsync(const FString& InCmdLineParams, FOutputDevice &Ar, void*& OutReadPipe, void*& OutWritePipe, bool bSkipBuildUBT)
{
	...
	if (!bSkipBuild)
	{
		...
		if (!bSuccessfullyBuiltUBTOnce)
		{
			...
			else
			{
				...
				UE_LOG(LogCXZDesktopPlatform, Warning, TEXT("Failed to compile UnrealBuildTool (project file is '%s', exe path is '%s')"), *GetUnrealBuildToolProjectFileName(FPaths::RootDir()), *ExecutableFileName);
				return FProcHandle();
			}
		}
	}
	...
	if (!ProcHandle.IsValid())
	{
		UE_LOG(LogCXZDesktopPlatform, Warning, TEXT("Failed to launch UnrealBuildTool (exe path is '%s')"), *ExecutableFileName);
...
bool FCXZDesktopPlatformBase::RunUnrealBuildTool(const FText& Description, const FString& RootDir, const FString& Arguments, FFeedbackContext* Warn)
{
	int32 ExitCode;
	return static_cast<ICXZDesktopPlatform*>(this)->RunUnrealBuildTool(Description, RootDir, Arguments, Warn, ExitCode);
}
...
bool IsTargetInfoValid(const TArray<FCXZTargetInfo>& Targets, const FString& SourceDir, const FDateTime& LastModifiedTime)
{
	...
	// Create the state 
	TSet<FString> RemainingTargetNames;
	for (const FCXZTargetInfo& Target : Targets)
	{
...
const TArray<FCXZTargetInfo>& FCXZDesktopPlatformBase::GetTargetsForProject(const FString& ProjectFile) const
{
	...
	const TArray<FCXZTargetInfo>* Targets = ProjectFileToTargets.Find(NormalizedProjectFile);
	if (Targets != nullptr)
	{
		return *Targets;
	}
	...
	if(StatData.bIsValid)
	{
		...
		TArray<FCXZTargetInfo> NewTargets;
	...
	{
		return ProjectFileToTargets.Add(NormalizedProjectFile, TArray<FCXZTargetInfo>());
	}
	...
	FString Output;
	int32 ReturnCode = 0;
	if (!FPaths::FileExists(GetUnrealBuildToolExecutableFilename(FPaths::RootDir())) || !const_cast<FCXZDesktopPlatformBase*>(this)->InvokeUnrealBuildToolSync(Arguments, *GLog, true, ReturnCode, Output) || ReturnCode != 0)
	{
		...
		if (!GIsBuildMachine)
		{
			const_cast<FCXZDesktopPlatformBase*>(this)->InvokeUnrealBuildToolSync(Arguments, *GLog, false, ReturnCode, Output);
		}
	}
	...
	{
		UE_LOG(LogCXZDesktopPlatform, Warning, TEXT("Unable to read target info for %s"), (ProjectFile.Len() == 0)? TEXT("engine") : *ProjectFile);
	}
...
const TArray<FCXZTargetInfo>& FCXZDesktopPlatformBase::GetTargetsForCurrentProject() const
{
	return GetTargetsForProject(FPaths::GetProjectFilePath());
}

FString FCXZDesktopPlatformBase::GetDefaultProjectCreationPath()
{
	// My Documents
...
void FCXZDesktopPlatformBase::ReadLauncherInstallationList()
{
...
void FCXZDesktopPlatformBase::CheckForLauncherEngineInstallation(const FString &AppId, const FString &Identifier, TMap<FString, FString> &OutInstallations)
{
...
int32 FCXZDesktopPlatformBase::ParseReleaseVersion(const FString &Version)
{
	TCHAR *End;
...
TSharedPtr<FJsonObject> FCXZDesktopPlatformBase::LoadProjectFile(const FString &FileName)
{
...
bool FCXZDesktopPlatformBase::SaveProjectFile(const FString &FileName, TSharedPtr<FJsonObject> Object)
{
	FString FileContents;
...
const FUProjectDictionary &FCXZDesktopPlatformBase::GetCachedProjectDictionary(const FString& RootDir)
{
	FString NormalizedRootDir = RootDir;
	FPaths::NormalizeDirectoryName(NormalizedRootDir);
...
void FCXZDesktopPlatformBase::GetProjectBuildProducts(const FString& ProjectDir, TArray<FString> &OutFileNames, TArray<FString> &OutDirectoryNames)
{
...
FString FCXZDesktopPlatformBase::GetEngineSavedConfigDirectory(const FString& Identifier)
{
...
bool FCXZDesktopPlatformBase::EnumerateProjectsKnownByEngine(const FString &Identifier, bool bIncludeNativeProjects, TArray<FString> &OutProjectFileNames)
{
	...
	UE_LOG(LogCXZDesktopPlatform, Log, TEXT("Enumerating Projects From Engine Ver: %s"), *Identifier);

	UE_LOG(LogCXZDesktopPlatform, Log, TEXT("Looking for directories to scan from : %s"), *GameAgnosticConfigDir);
	...
	if (!FConfigCacheIni::LoadExternalIniFile(GameAgnosticConfig, TEXT("EditorSettings"), NULL, *GameAgnosticConfigDir, false))
	{
		...
		UE_LOG(LogCXZDesktopPlatform, Log, TEXT("%s not found, looking for directories in %s"), *PreviousConfigDir , *GameAgnosticConfigDir);
	...
	{
		UE_LOG(LogCXZDesktopPlatform, Log, TEXT("Config not found"));
	}

	if(Section != NULL)
	{
		UE_LOG(LogCXZDesktopPlatform, Log, TEXT("Searching for previously created project directories..."));
		...
		for(int Idx = 0; Idx < AdditionalDirectories.Num(); Idx++)
		{
			...
			UE_LOG(LogCXZDesktopPlatform, Log, TEXT("Found directory: \"%s\""), *AdditionalDirectories[Idx]);
		...
		for(int Idx = 0; Idx < RecentlyOpenedFiles.Num(); Idx++)
		{
			...
			UE_LOG(LogCXZDesktopPlatform, Log, TEXT("Found project \"%s\" in recently opened files"), *RecentlyOpenedFiles[Idx]);
	...
	for(int Idx = 0; Idx < SearchDirectories.Num(); Idx++)
	{
		...
		for(int32 FolderIdx = 0; FolderIdx < ProjectFolders.Num(); FolderIdx++)
		{
			...
			for(int32 FileIdx = 0; FileIdx < ProjectFiles.Num(); FileIdx++)
			{
				...
				UE_LOG(LogCXZDesktopPlatform, Log, TEXT("Found project \"%s\" in previously created project directory"), *ProjName);
	...
	UE_LOG(LogCXZDesktopPlatform, Log, TEXT("Searcing for projects in .uprojectdirs"));
	...
	if(bIncludeNativeProjects)
	{
		...
		for(int Idx = 0; Idx < NativeProjectPaths.Num(); Idx++)
		{
			...
			{
				UE_LOG(LogCXZDesktopPlatform, Log, TEXT("Found project \"%s\" in .uprojectdirs"), *NativeProjectPaths[Idx]);
...
bool FCXZDesktopPlatformBase::BuildUnrealBuildTool(const FString& RootDir, FOutputDevice& Ar)
{
...
bool FCXZDesktopPlatformBase::ReadTargetInfo(const FString& FileName, TArray<FCXZTargetInfo>& Targets)
{
	// Read the file to a string
	FString Contents;
...
FString FCXZDesktopPlatformBase::GetUserDir(const FString& Identifier)
{
	// Get the engine root directory
	FString RootDir;
	if (!GetEngineRootDirFromIdentifier(Identifier, RootDir))
...
FString FCXZDesktopPlatformBase::GetLegacyEngineSavedConfigDirectory(const FString& Identifier)
{
	const FString UserDir = GetUserDir(Identifier);
	if (!UserDir.IsEmpty())
	{
...
FString FCXZDesktopPlatformBase::GetUnrealBuildToolProjectFileName(const FString& RootDir) const
{
	return FPaths::ConvertRelativePathToFull(RootDir / TEXT("Engine/Source/Programs/UnrealBuildTool/UnrealBuildTool.csproj"));
}

FString FCXZDesktopPlatformBase::GetUnrealBuildToolExecutableFilename(const FString& RootDir) const
{
	FConfigFile Config;
