...
#include "CXZDesktopPlatformWindows.h"
#include "CXZDesktopPlatformPrivate.h"
#include "Misc/FeedbackContextMarkup.h"
#include "CXZWindowsNativeFeedbackContext.h"
...
bool FCXZDesktopPlatformWindows::OpenFileDialog( const void* ParentWindowHandle, const FString& DialogTitle, const FString& DefaultPath, const FString& DefaultFile, const FString& FileTypes, uint32 Flags, TArray<FString>& OutFilenames, int32& OutFilterIndex )
{
...
bool FCXZDesktopPlatformWindows::OpenFileDialog( const void* ParentWindowHandle, const FString& DialogTitle, const FString& DefaultPath, const FString& DefaultFile, const FString& FileTypes, uint32 Flags, TArray<FString>& OutFilenames)
{
...
bool FCXZDesktopPlatformWindows::SaveFileDialog(const void* ParentWindowHandle, const FString& DialogTitle, const FString& DefaultPath, const FString& DefaultFile, const FString& FileTypes, uint32 Flags, TArray<FString>& OutFilenames)
{
...
bool FCXZDesktopPlatformWindows::MakeDirectory(const FString& NewDirectory)
{
	const TCHAR* tDest = *NewDirectory;
	return IFileManager::Get().MakeDirectory(tDest);
}
bool FCXZDesktopPlatformWindows::RenameFile(const FString& Src,const FString& Dest)
{
	const TCHAR* tDest = *Dest;
	const TCHAR* tSrc = *Src;
	return IFileManager::Get().Move(tDest,tSrc);
}

bool FCXZDesktopPlatformWindows::OpenDirectoryDialog(const void* ParentWindowHandle, const FString& DialogTitle, const FString& DefaultPath, FString& OutFolderName)
{
	FCXZScopedSystemModalMode SystemModalScope;
...
bool FCXZDesktopPlatformWindows::OpenFontDialog(const void* ParentWindowHandle, FString& OutFontName, float& OutHeight, ECXZFontImportFlags& OutFlags)
{	
	FCXZScopedSystemModalMode SystemModalScope;
	...
	if ( bSuccess )
	{
		...
		auto FontFlags = ECXZFontImportFlags::None;
		if ( lf.lfUnderline )
		{
			FontFlags |= ECXZFontImportFlags::EnableUnderline;
		}
		if ( lf.lfItalic )
		{
			FontFlags |= ECXZFontImportFlags::EnableItalic;
		}
		if ( lf.lfWeight == FW_BOLD )
		{
			FontFlags |= ECXZFontImportFlags::EnableBold;
		}
	...
	{
		UE_LOG(LogCXZDesktopPlatform, Warning, TEXT("Error reading results of font dialog."));
	}
...
bool FCXZDesktopPlatformWindows::FileDialogShared(bool bSave, const void* ParentWindowHandle, const FString& DialogTitle, const FString& DefaultPath, const FString& DefaultFile, const FString& FileTypes, uint32 Flags, TArray<FString>& OutFilenames, int32& OutFilterIndex)
{
	FCXZScopedSystemModalMode SystemModalScope;
	...
	if (SUCCEEDED(::CoCreateInstance(bSave ? CLSID_FileSaveDialog : CLSID_FileOpenDialog, nullptr, CLSCTX_INPROC_SERVER, bSave ? IID_IFileSaveDialog : IID_IFileOpenDialog, IID_PPV_ARGS_Helper(&FileDialog))))
	{
		...
		else
		{
			...
			if (Flags & ECXZFileDialogFlags::Multiple)
			{
				DWORD dwFlags = 0;
				FileDialog->GetOptions(&dwFlags);
...
bool FCXZDesktopPlatformWindows::RegisterEngineInstallation(const FString &RootDir, FString &OutIdentifier)
{
	bool bRes = false;
...
void FCXZDesktopPlatformWindows::EnumerateEngineInstallations(TMap<FString, FString> &OutInstallations)
{
...
bool FCXZDesktopPlatformWindows::IsSourceDistribution(const FString &RootDir)
{
	...
	return FCXZDesktopPlatformBase::IsSourceDistribution(RootDir);
}

bool FCXZDesktopPlatformWindows::VerifyFileAssociations()
{
...
bool FCXZDesktopPlatformWindows::UpdateFileAssociations()
{
	TIndirectArray<FRegistryRootedKey> Keys;
	GetRequiredRegistrySettings(Keys);
...
bool FCXZDesktopPlatformWindows::OpenProject(const FString &ProjectFileName)
{
...
bool FCXZDesktopPlatformWindows::RunUnrealBuildTool(const FText& Description, const FString& RootDir, const FString& Arguments, FFeedbackContext* Warn, int32& OutExitCode)
{
	OutExitCode = 1;
...
bool FCXZDesktopPlatformWindows::IsUnrealBuildToolRunning()
{
...
FFeedbackContext* FCXZDesktopPlatformWindows::GetNativeFeedbackContext()
{
	static FWindowsNativeFeedbackContext FeedbackContext;
	return &FeedbackContext;
}

FString FCXZDesktopPlatformWindows::GetUserTempPath()
{
	return FString(FPlatformProcess::UserTempDir());
}

void FCXZDesktopPlatformWindows::GetRequiredRegistrySettings(TIndirectArray<FRegistryRootedKey> &RootedKeys)
{
...
int32 FCXZDesktopPlatformWindows::GetShellIntegrationVersion(const FString &FileName)
{
