...
#include "CXZTargetReceipt.h"

DEFINE_LOG_CATEGORY_STATIC(LogCXZTargetReceiptBuildWorker, Log, All);

// This needs to match the version exported in DerivedDataBuildWorker.Build.cs 
const FGuid FCXZTargetReceiptBuildWorker::WorkerReceiptVersion(TEXT("dab5352e-a5a7-4793-a7a3-1d4acad6aff2"));

FCXZTargetReceiptBuildWorker::FWorkerPath::FWorkerPath(FStringView InLocalPathRoot, FStringView InLocalPathSuffix, FStringView InRemotePathRoot, FStringView InRemotePathSuffix)
{
...
{
public:
	FPopulateWorkerFromTargetReceiptTask(const TCHAR* InTargetReceiptFilePath, FCXZTargetReceiptBuildWorker* InWorker)
...
private:
	...
	FCXZTargetReceiptBuildWorker* Worker;
};

class FCXZTargetReceiptBuildWorkerFactory final : public UE::DerivedData::IBuildWorkerFactory
{
public:
	const FCXZTargetReceiptBuildWorker* GetWorker()
	{
		return (FCXZTargetReceiptBuildWorker*)((uintptr_t)this - offsetof(class FCXZTargetReceiptBuildWorker, InternalFactory));
	}

	void Build(UE::DerivedData::FBuildWorkerBuilder& Builder) final
	{
		const FCXZTargetReceiptBuildWorker* Worker = GetWorker();
	...
	void FindFileData(TConstArrayView<FIoHash> RawHashes, UE::DerivedData::IRequestOwner& Owner, UE::DerivedData::FOnBuildWorkerFileDataComplete&& OnComplete) final
	{
		if (OnComplete)
		{
			...
			for (const FIoHash& RawHash : RawHashes)
			{
				for (int32 ExecutableIndex = 0; ExecutableIndex < Worker->ExecutablePaths.Num(); ++ExecutableIndex)
				{
					const FCXZTargetReceiptBuildWorker::FWorkerPathMeta& Meta = Worker->ExecutableMeta[ExecutableIndex];
					if (Meta.Key == RawHash)
					{
...
FCXZTargetReceiptBuildWorker::FCXZTargetReceiptBuildWorker(const TCHAR* TargetReceiptFilePath)
: bAbortRequested(false)
, bEnabled(false)
{
#if WITH_EDITOR
	static_assert(sizeof(FCXZTargetReceiptBuildWorkerFactory) == sizeof(IPureVirtual), "FCXZTargetReceiptBuildWorkerFactory type must always compile to something equivalent to an IPureVirtual size.");
	FCXZTargetReceiptBuildWorkerFactory* WorkerFactory = GetWorkerFactory();
	new(WorkerFactory) FCXZTargetReceiptBuildWorkerFactory();
...
FCXZTargetReceiptBuildWorker::~FCXZTargetReceiptBuildWorker()
{
#if WITH_EDITOR
	...
	WorkerFactory->~FCXZTargetReceiptBuildWorkerFactory();
#endif
}

FCXZTargetReceiptBuildWorkerFactory* FCXZTargetReceiptBuildWorker::GetWorkerFactory()
{
	return (FCXZTargetReceiptBuildWorkerFactory*)&InternalFactory;
}

bool FCXZTargetReceiptBuildWorker::TryAddExecutablePath(FStringView Path)
{
	if (Path.IsEmpty())
	{
		UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Empty executable path encountered while populating worker '%s'."), *Name, Path.Len(), Path.GetData());
		return false;
	}

	if (!Path.StartsWith(TEXT("$(")))
	{
		UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Absolute or unprefixed executable path variable encountered while populating worker '%s': '%.*s'"), *Name, Path.Len(), Path.GetData());
		return false;
	}
	...
	{
		UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Malformed executable path variable encountered while populating worker '%s': '%.*s'"), *Name, Path.Len(), Path.GetData());
		return false;
	}
	...
	else
	{
		...
		{
			UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Unknown executable path variable encountered while populating worker '%s': '%s'"), *Name, *TempVariableName);
			return false;
		}
...
void FCXZTargetReceiptBuildWorker::PopulateWorkerFromReceipt(const FString& TargetReceiptFilePath)
{
	if (bAbortRequested)
	{
		return;
	...
	{
		UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Unknown prefix for target receipt path: '%s'"), *TargetReceiptFilePath);
		return;
	}
	...
	FCXZTargetReceipt CXZTargetReceipt;
	if (!CXZTargetReceipt.Read(FinalTargetReceiptFilePath, false))
	{
		UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Target receipt failed to parse: '%s'"), *TargetReceiptFilePath);
		return;
	}
	...
	Name = CXZTargetReceipt.TargetName;
	Platform = CXZTargetReceipt.Platform;
	if (!TryAddExecutablePath(CXZTargetReceipt.Launch))
	{
		return;
	}
	...
	for (const FReceiptProperty& Property : CXZTargetReceipt.AdditionalProperties)
	{
		...
		else if (Property.Name == TEXT("DerivedDataBuildWorkerBuildSystemVersion"))
		{
			...
			{
				UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Target receipt contains inline build system version guid that could not be parsed: '%s'"), *Property.Value);
				return;
			}
		}
		else if (Property.Name == TEXT("DerivedDataBuildWorkerFunctionVersion"))
		{
			...
			{
				UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Target receipt contains inline function version declaration that could not be parsed: '%s'"), *Property.Value);
				return;
			}
			...
			{
				UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Target receipt contains inline function version guid that could not be parsed: '%s'"), *Property.Value);
				return;
			}
		...
		else if (Property.Name == TEXT("UnstagedRuntimeDependency"))
		{
			...
			{
				UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Target receipt referenced an unstaged dependency that was not usable: '%s'"), *Property.Value);
				return;
			}
		}
	}
	...
	for (const FRuntimeDependency& RuntimeDependency : CXZTargetReceipt.RuntimeDependencies)
	{
		...
		{
			UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Target receipt referenced a runtime dependency that was not usable: '%s'"), *RuntimeDependency.Path);
			return;
		}
	}
	...
	for (const FWorkerPath& ExecutablePath : ExecutablePaths)
	{
		...
		{
			UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Executable file not present: '%s'"), *LocalExecutablePath);
			return;
		}
		++MetaIndex;
	}
	...
	else
	{
		...
		if (FPaths::FileExists(FinalPublishedVersionFilePath))
		{
			...
			{
				UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Could not read published version file: '%s'"), *CXZTargetReceipt.Launch);
				return;
			}
		}
		else
		{
			...
			if (CXZTargetReceipt.Platform != FPlatformProcess::GetBinariesSubdirectory())
			{
				UE_LOG(LogCXZTargetReceiptBuildWorker, Log, TEXT("Target lacked published version information and isn't made for the current platform: '%s'"), *CXZTargetReceipt.Launch);
				return;
			}
			...
			if (VersionFileModificationTime < NewestExecutableModificationTime)
			{
				...
				{
					UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Failed to launch worker: '%s'"), *CXZTargetReceipt.Launch);
					return;
				}
				...
				while (FPlatformProcess::IsProcRunning(WorkerProcHandle))
				{
					...
					{
						UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Aborted launch of worker: '%s'"), *CXZTargetReceipt.Launch);
				...
				{
					UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Worker did not produce version file: '%s'"), *CXZTargetReceipt.Launch);
					return;
				}
			}
			...
			{
				UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Worker did not produce readable version file: '%s'"), *CXZTargetReceipt.Launch);
				return;
			}
		}
		...
		{
			UE_LOG(LogCXZTargetReceiptBuildWorker, Warning, TEXT("Invalid build system version in version file: '%s'"), *VersionFilePath);
			return;
		}
